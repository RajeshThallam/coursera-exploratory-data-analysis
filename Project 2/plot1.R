...
#         2. Explore total emissions from PM2.5 and determing if it decreased 
#            in the United States from 1999 to 2008
#         3. Draw bar plot between total emissions (y-axis) and year (x-axis)
...
draw.plot1 <- function() {
  ...
  # aggregating total emissions over the year
  p("Preparing plot data by aggregating total emissions for each year")
  ...
  # draw bar plot between total emissions from all sources for each of 
  # the years
  p("Draw bar plot for total emissions from all sources for each year")
  ...
  barplot(
    ...
    main=expression('Total PM'[2]*' emission for each year')
  )
